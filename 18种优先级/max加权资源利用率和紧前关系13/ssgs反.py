...
if __name__ == '__main__':
    ...
    jobs = psp1.jobs
    job_dicts = []
    for job in psp1.jobs:
        set_number = 0.7*len(job.preses)
        reses_0 = 0
            # 四种可更新资源
        for i in range(len(psp1.res_tops)):
                # job.modes[0].reses是活动的需求量，self.prj.res_tops是资源总量
            reses_0 = job.modes[0].reses[i]*(1/(psp1.res_tops[i])) + reses_0
            reses_f = 0.3*reses_0
            wrup_val = set_number + reses_f
        job_dict = {'job_nb': job.nb, 'wrup_val': wrup_val}
        job_dicts.append(job_dict)
    # 对所有字典按照资源需求量排序
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['wrup_val'], reverse=True)
    # 输出排序后的资源需求情况
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'wrup_val:', job_dict['wrup_val'])    # psp1.initialize()
    # 添加入紧前关系
    psp1.set_job_preses()
