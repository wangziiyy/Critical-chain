...
if __name__ == '__main__':
    ...
    for job in jobs:
        if len(job.preses) == 0:
            job_dict = {'job_nb': job.nb, 'wacru_val': 0}
            job_dicts.append(job_dict)
        else:
            gi_0 = 0
            for job_0 in job.preses:
                float_val = psp1.jobs[job_0].cpmTime['float']
                gi_0 = 1/(1+float_val) + gi_0
            gi = 0.7 * gi_0
            reses_d = 0
            for i in range(len(psp1.res_tops)):
                reses_d = job.modes[0].reses[i] * (1 / (psp1.res_tops[i])) + reses_d
            resource_use = 0.3 * reses_d
            wacru_val = gi + resource_use
            job_dict = {'job_nb': job.nb, 'wacru_val': wacru_val}
            job_dicts.append(job_dict)
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['wacru_val'],reverse = True)
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'dur_sum:', job_dict['wacru_val'])
    # psp1.initialize()
    # 添加入紧前关系
