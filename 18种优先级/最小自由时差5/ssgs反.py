...
if __name__ == '__main__':
    ...
    for job in jobs:
        if len(job.preses) == 0:
            job_dict = {'job_nb': job.nb, 'ff_val':1000}
            job_dicts.append(job_dict)
        else:
            est = 1000
            for nb in job.preses:
                if psp1.jobs[nb].cpmTime['start'] < est:
                    est = psp1.jobs[nb].cpmTime['start']
            ff_val = est - job.cpmTime['end']
            job_dict = {'job_nb': job.nb, 'ff_val': ff_val}
            job_dicts.append(job_dict)
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['ff_val'])
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'ff_val:', job_dict['ff_val']) 
