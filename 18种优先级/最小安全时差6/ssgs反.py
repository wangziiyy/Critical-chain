...
if __name__ == '__main__':
    ...
    for job in jobs:
        if len(job.sucs) == 0:
            job_dict = {'job_nb': job.nb, 'msf_val':1000}
            job_dicts.append(job_dict)
        else:
            lft = 0
            for nb in job.sucs:
                if psp1.jobs[nb].cpmTime['lateEnd'] > lft:
                    lft = psp1.jobs[nb].cpmTime['lateEnd']
            msf_val = job.cpmTime['lateStart'] - lft
            job_dict = {'job_nb': job.nb, 'msf_val': msf_val}
            job_dicts.append(job_dict)
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['msf_val'])
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'msf_val:', job_dict['msf_val'])     # 添加入紧前关系
    # 生成cpmTime——最早开始，最早完成，最晚开始，最晚结束，不会影响job.start和job.end
    # 创建cpm实例
    #cpm = Cpm(psp1)
    #cpm.cpm_calculate()
