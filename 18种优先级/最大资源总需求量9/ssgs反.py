...
if __name__ == '__main__':
    ...
    for job in psp1.jobs:
        ...
        job_dict = {'job_nb': job.nb, 'dur_reses': dur_reses}
        job_dicts.append(job_dict)
    # 对所有字典按照资源需求量排序
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['dur_reses'], reverse=True)
    # 输出排序后的资源需求情况
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'dur_reses:', job_dict['dur_reses'])
    # psp1.initialize()
    # 添加入紧前关系
    psp1.set_job_preses()
