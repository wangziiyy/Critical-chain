...
class ResourcePool:
    ...
    def get_activity_resource(self, resource_type, resource_count):
        ...
        activity_list_sorted = sorted(activity_list, key=lambda x: x['sum_reses'])
# 将排序后的活动名称存入列表中
...
# 创建 activity 列表通过这个代码能否把活动的紧前关系紧后关系给表示出来
activities = []
activity_list = []
# 遍历字典并创建 activity
for item in data:
    ...
    for activity in activity_list:
        if activity["start_time"] == i:
            ...
            if selected_resource is None:
                ...
                #print("Activity", activity["name"], "starts",selected_resource)
                print("Activity", activity["name"], "starts", selected_resource["activity_name"])
            print("Resource pool:", resource_pool.resource_list)
