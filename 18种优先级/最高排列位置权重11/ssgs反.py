...
if __name__ == '__main__':
    ...
    for job in jobs:
        if len(job.preses) == 0:
            job_dict = {'job_nb': job.nb, 'dur_sum': 0}
            job_dicts.append(job_dict)
        else:
            sum_dur_sum = 0
            for nb in job.preses:
                sucs_dur_sum = psp1.jobs[nb].modes[0].dur + sum_dur_sum
            dur_sum = job.modes[0].dur + sucs_dur_sum
            job_dict = {'job_nb': job.nb, 'dur_sum': dur_sum}
            job_dicts.append(job_dict)
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['dur_sum'],reverse = True)
    for job_dict in sorted_job_dicts:
       print('job_nb:', job_dict['job_nb'], 'dur_sum:', job_dict['dur_sum']) 
    # psp1.initialize()
    # 添加入紧前关系
    #psp1.set_job_preses()
