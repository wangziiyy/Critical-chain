...
if __name__ == '__main__':
    ...
    for job in jobs:
        if len(job.preses) == 0:
            job_dict = {'job_nb': job.nb, 'lfs': 1000}
            job_dicts.append(job_dict)
        else:
            set_0 = 0
            for job_0 in job.preses:
                set_0 = len(psp1.jobs[job_0].preses) + set_0
            set_1 = set_0+len(job.preses)
            lenth[job.nb] = 1/set_1
            TF = job.cpmTime['lateStart'] - job.cpmTime['start']
            avg = TF * lenth[job.nb]
            job_dict = {'job_nb': job.nb, 'lfs': avg}
            job_dicts.append(job_dict)
    sorted_job_dicts = sorted(job_dicts, key=lambda x: x['lfs'])
    for job_dict in sorted_job_dicts:
      print('job_nb:', job_dict['job_nb'], 'lfs:', job_dict['lfs']) 
